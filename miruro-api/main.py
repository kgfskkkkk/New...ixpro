import asyncio, base64, json, gzip, httpx, os, re
...
# ---------------- Cloudflare bypass (optional solver hook) ----------------
# Miruro's pipe endpoint is behind a Cloudflare challenge. The API tries the
# direct request first; on a CF challenge it falls back to a configurable
# FlareSolverr-style solver. Set CF_SOLVER_URL to enable (e.g.
# http://localhost:8191 for a self-hosted FlareSolverr, or any compatible
# hosted service). CF_SOLVER_MODE controls the request shape:
#   flaresolverr - POST {CF_SOLVER_URL}{CF_SOLVER_PATH} with the FlareSolverr
#                  cmd payload; reads solution.response
#   simple       - POST {CF_SOLVER_URL}{CF_SOLVER_PATH} with {"url": ...};
#                  reads response/content/html fields defensively
CF_SOLVER_URL = os.getenv("CF_SOLVER_URL", "").strip().rstrip("/")
CF_SOLVER_PATH = os.getenv("CF_SOLVER_PATH", "/v1")
CF_SOLVER_MODE = os.getenv("CF_SOLVER_MODE", "flaresolverr").strip().lower()
CF_SOLVER_TIMEOUT = float(os.getenv("CF_SOLVER_TIMEOUT", "90"))
CF_SOLVER_HEADLESS = os.getenv("CF_SOLVER_HEADLESS", "true").lower() == "true"
CF_SOLVER_SESSION = os.getenv("CF_SOLVER_SESSION", "miruro-pipe")

# ---------------- Token-based scraping API bypass ----------------
# Some scraping APIs (ScraperAPI, ScrapingBee, ZenRows) route the request
# through non-blocked IPs and solve the Cloudflare challenge for you. Set
# CF_SCRAPER_API_KEY + CF_SCRAPER_PROVIDER to use one instead of (or as a
# fallback to) a self-hosted FlareSolverr. CF_SCRAPER_ENDPOINT / KEY_PARAM
# override the preset so any other GET-style scraping service works too.
CF_SCRAPER_API_KEY = os.getenv("CF_SCRAPER_API_KEY", "").strip()
CF_SCRAPER_PROVIDER = os.getenv("CF_SCRAPER_PROVIDER", "scraperapi").strip().lower()
# Optional second scraper (tried as a fallback when the first one fails).
CF_SCRAPER_API_KEY_2 = os.getenv("CF_SCRAPER_API_KEY_2", "").strip()
CF_SCRAPER_PROVIDER_2 = os.getenv("CF_SCRAPER_PROVIDER_2", "").strip().lower()
CF_SCRAPER_ENDPOINT = os.getenv("CF_SCRAPER_ENDPOINT", "").strip().rstrip("/")
CF_SCRAPER_KEY_PARAM = os.getenv("CF_SCRAPER_KEY_PARAM", "").strip()
# Paid boosters: enable JS rendering / residential premium proxies. Miruro's
# Cloudflare blocks datacenter IPs, so free tiers usually fail with an IP-block
# error — these flags map to each provider's premium params (paid plans).
CF_SCRAPER_RENDER = os.getenv("CF_SCRAPER_RENDER", "false").lower() == "true"
CF_SCRAPER_PREMIUM = os.getenv("CF_SCRAPER_PREMIUM", "false").lower() == "true"

SCRAPER_PRESETS = {
    "scraperapi": {"endpoint": "https://api.scraperapi.com/", "key_param": "api_key"},
    "scrapingbee": {"endpoint": "https://app.scrapingbee.com/api/v1/", "key_param": "api_key"},
    "zenrows": {"endpoint": "https://api.zenrows.com/v1/", "key_param": "apikey"},
}

# ---------------- Bright Data Web Unlocker ----------------
# Handles anti-bot/CAPTCHA server-side. Uses a Bearer API key + zone name.
CF_BRIGHTDATA_API_KEY = os.getenv("CF_BRIGHTDATA_API_KEY", "").strip()
CF_BRIGHTDATA_ZONE = os.getenv("CF_BRIGHTDATA_ZONE", "web_unlocker1").strip()

# ---------------- Turnstile solver (2Captcha / CapSolver) ----------------
# Solves Cloudflare Turnstile and returns a token. Best-effort: works for
# standalone Turnstile widgets; Cloudflare "challenge page" mode needs a real
# browser (use FlareSolverr for that).
CF_TURNSTILE_KEY = os.getenv("CF_TURNSTILE_KEY", "").strip()
CF_TURNSTILE_PROVIDER = os.getenv("CF_TURNSTILE_PROVIDER", "2captcha").strip().lower()
...
async def _fetch_raw_episodes(anilist_id: int) -> dict:
    ...
    if res.status_code != 200:
        raise HTTPException(status_code=res.status_code, detail={"status": res.status_code, "body": res.text[:500], "headers": res.headers})
    try:
        data = _decode_pipe_response(res.text.strip())
    except ValueError as e:
        raise HTTPException(status_code=502, detail=f"Pipe response invalid (challenge or upstream error): {e}")
...
class _PipeResponse:
    """Minimal response object; the callers only use status_code / text."""
...
def _is_cf_challenge(res) -> bool:
    text = str(getattr(res, "text", "") or "")
    return ("challenge-platform" in text or "Attention Required" in text
            or "Just a moment" in text or "cf_chl_" in text)


def _extract_solver_body(data: dict) -> str:
    """Defensively pull the fetched page out of a solver response."""
    if not isinstance(data, dict):
        return ""
    solution = data.get("solution")
    if isinstance(solution, dict):
        for key in ("response", "content", "html", "body"):
            val = solution.get(key)
            if isinstance(val, str) and len(val) > 50:
                return val
    for key in ("response", "content", "html", "body", "result"):
        val = data.get(key)
        if isinstance(val, str) and len(val) > 50:
            return val
    return ""
...
async def _cf_solver_fetch(url: str) -> _PipeResponse:
    """Solve a CF-blocked URL through the configured solver."""
    if not CF_SOLVER_URL:
        raise HTTPException(status_code=502, detail="CF_SOLVER_URL is not configured")
    endpoint = f"{CF_SOLVER_URL}{CF_SOLVER_PATH}"
    if CF_SOLVER_MODE == "simple":
        payload = {"url": url, "headless": CF_SOLVER_HEADLESS}
    else:
        payload = {
            "cmd": "request.get",
            "url": url,
            "maxTimeout": int(CF_SOLVER_TIMEOUT * 1000),
            "headless": CF_SOLVER_HEADLESS,
            "session": CF_SOLVER_SESSION,
        }
    try:
        async with httpx.AsyncClient(timeout=CF_SOLVER_TIMEOUT + 15) as client:
            res = await client.post(endpoint, json=payload)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"CF solver unreachable: {e}")
    if res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"CF solver HTTP {res.status_code}: {res.text[:200]}")
    data = res.json()
    body = _extract_solver_body(data)
    if not body:
        msg = data.get("message") or data.get("error") or str(data)[:200]
        raise HTTPException(status_code=502, detail=f"CF solver failed: {msg}")
    if _is_cf_challenge(_PipeResponse(200, body)):
        raise HTTPException(status_code=502, detail="CF solver returned a challenge page (its IP is likely blocked too)")
    return _PipeResponse(200, body)


async def _scraper_fetch(url: str, key: str = None, provider: str = None) -> _PipeResponse:
    """Fetch a CF-blocked URL through a token-based scraping API."""
    key = key or CF_SCRAPER_API_KEY
    provider = (provider or CF_SCRAPER_PROVIDER).strip().lower()
    if not key:
        raise HTTPException(status_code=502, detail="scraper API key is not configured")
    preset = SCRAPER_PRESETS.get(provider)
    endpoint = CF_SCRAPER_ENDPOINT or (preset["endpoint"] if preset else "")
    key_param = CF_SCRAPER_KEY_PARAM or (preset["key_param"] if preset else "api_key")
    if not endpoint:
        raise HTTPException(status_code=502, detail=f"Unknown CF_SCRAPER_PROVIDER: {provider}")
    params = {key_param: key, "url": url}
    if CF_SCRAPER_RENDER:
        if provider == "scrapingbee":
            params["render_js"] = "true"
        elif provider == "zenrows":
            params["js_render"] = "true"
        else:
            params["render"] = "true"  # scraperapi
    if CF_SCRAPER_PREMIUM:
        if provider in ("zenrows", "scrapingbee"):
            params["premium_proxy"] = "true"
        else:
            params["premium"] = "true"  # scraperapi
    try:
        async with httpx.AsyncClient(timeout=CF_SOLVER_TIMEOUT + 15) as client:
            res = await client.get(endpoint, params=params, follow_redirects=True)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Scraper unreachable: {e}")
    body = res.text or ""
    if res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Scraper HTTP {res.status_code}: {body[:200]}")
    if _is_cf_challenge(_PipeResponse(res.status_code, body)):
        raise HTTPException(status_code=502, detail="Scraper returned a challenge page (key/plan may not bypass CF)")
    return _PipeResponse(200, body)


async def _brightdata_fetch(url: str) -> _PipeResponse:
    """Fetch a CF-blocked URL through Bright Data Web Unlocker (Bearer token)."""
    if not CF_BRIGHTDATA_API_KEY:
        raise HTTPException(status_code=502, detail="CF_BRIGHTDATA_API_KEY is not configured")
    payload = {"zone": CF_BRIGHTDATA_ZONE, "url": url, "format": "raw"}
    headers = {"Authorization": f"Bearer {CF_BRIGHTDATA_API_KEY}", "Content-Type": "application/json"}
    try:
        async with httpx.AsyncClient(timeout=CF_SOLVER_TIMEOUT + 15) as client:
            res = await client.post("https://api.brightdata.com/request", json=payload, headers=headers)
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Bright Data unreachable: {e}")
    try:
        data = res.json()
    except Exception:
        raise HTTPException(status_code=502, detail=f"Bright Data HTTP {res.status_code}: {res.text[:200]}")
    if res.status_code != 200 or "body" not in data:
        raise HTTPException(status_code=502, detail=f"Bright Data error: {data}")
    body = data.get("body", "")
    if isinstance(body, (dict, list)):
        body = json.dumps(body)
    if _is_cf_challenge(_PipeResponse(int(data.get("status_code", 200)), body)):
        raise HTTPException(status_code=502, detail="Bright Data returned a challenge page")
    return _PipeResponse(200, body)


def _extract_turnstile_sitekey(html: str) -> str:
    m = re.search(r'data-sitekey=["\']([0-9a-zA-Z_-]+)["\']', html or "")
    if m:
        return m.group(1)
    m = re.search(r'sitekey["\']?\s*[:=]\s*["\']([0-9a-zA-Z_-]+)["\']', html or "")
    if m:
        return m.group(1)
    return ""


async def _twocaptcha_turnstile(url: str, sitekey: str) -> str:
    payload = {
        "clientKey": CF_TURNSTILE_KEY,
        "task": {"type": "TurnstileTaskProxyless", "websiteURL": url, "websiteKey": sitekey},
    }
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post("https://api.2captcha.com/createTask", json=payload)
    data = r.json()
    if data.get("errorId") != 0 or not data.get("taskId"):
        raise HTTPException(status_code=502, detail=f"2Captcha createTask failed: {data}")
    task_id = data["taskId"]
    for _ in range(30):
        await asyncio.sleep(3)
        r = await client.post("https://api.2captcha.com/getTaskResult", json={"clientKey": CF_TURNSTILE_KEY, "taskId": task_id})
        d = r.json()
        if d.get("status") == "ready":
            return (d.get("solution") or {}).get("token", "")
        if d.get("errorId"):
            raise HTTPException(status_code=502, detail=f"2Captcha solve error: {d}")
    raise HTTPException(status_code=502, detail="2Captcha turnstile timed out")


async def _capsolver_turnstile(url: str, sitekey: str) -> str:
    payload = {
        "clientKey": CF_TURNSTILE_KEY,
    ...
    data = r.json()
    if data.get("errorId") != 0 or not data.get("taskId"):
        raise HTTPException(status_code=502, detail=f"CapSolver createTask failed: {data}")
    task_id = data["taskId"]
    for _ in range(30):
        await asyncio.sleep(3)
        r = await client.post("https://api.capsolver.com/getTaskResult", json={"clientKey": CF_TURNSTILE_KEY, "taskId": task_id})
        d = r.json()
        if d.get("status") == "ready":
            return (d.get("solution") or {}).get("token", "")
        if d.get("status") == "failed":
            raise HTTPException(status_code=502, detail=f"CapSolver solve failed: {d}")
    raise HTTPException(status_code=502, detail="CapSolver turnstile timed out")


async def _turnstile_fetch(url: str) -> _PipeResponse:
    """Solve Cloudflare Turnstile (2Captcha/CapSolver) and retry the request."""
    if not CF_TURNSTILE_KEY:
        raise HTTPException(status_code=502, detail="CF_TURNSTILE_KEY is not configured")
    async with AsyncSession(impersonate="chrome110") as client:
        res = await client.get(url, headers=HEADERS)
        html = res.text or ""
    sitekey = _extract_turnstile_sitekey(html)
    if not sitekey:
        raise HTTPException(status_code=502, detail="Could not extract Turnstile sitekey from challenge page")
    token = await (_capsolver_turnstile(url, sitekey) if CF_TURNSTILE_PROVIDER == "capsolver" else _twocaptcha_turnstile(url, sitekey))
    # Best-effort: replay with the token as a cf-turnstile-response form field
    # and cf_clearance cookie. Works for standalone widgets; Cloudflare
    # "challenge page" mode needs a real browser — use FlareSolverr for that.
    async with AsyncSession(impersonate="chrome110") as client:
        ...
        if r2.status_code == 200 and not _is_cf_challenge(r2):
            return _PipeResponse(200, r2.text)
    raise HTTPException(status_code=502, detail="Turnstile token obtained but target still challenged (challenge-page mode needs a browser; use FlareSolverr)")


async def _pipe_get(url: str) -> _PipeResponse:
    """GET a miruro pipe URL; on a Cloudflare challenge fall back to a bypass."""
    async with AsyncSession(impersonate="chrome110") as client:
        res = await client.get(url, headers=HEADERS)
    if res.status_code == 200:
        return _PipeResponse(200, res.text)
    blocked = res.status_code in (403, 429, 503) or _is_cf_challenge(res)
    if not blocked:
        return _PipeResponse(res.status_code, res.text)

    attempts = []
    scraper_specs = [(CF_SCRAPER_API_KEY, CF_SCRAPER_PROVIDER), (CF_SCRAPER_API_KEY_2, CF_SCRAPER_PROVIDER_2)]
    for key, prov in scraper_specs:
        if key:
            async def _scraper_call(u, _key=key, _prov=prov):
                return await _scraper_fetch(u, key=_key, provider=_prov)
            attempts.append((f"{prov} scraper", _scraper_call))
    if CF_BRIGHTDATA_API_KEY:
        attempts.append(("brightdata", _brightdata_fetch))
    if CF_TURNSTILE_KEY:
        attempts.append((f"{CF_TURNSTILE_PROVIDER} turnstile", _turnstile_fetch))
    if CF_SOLVER_URL:
        attempts.append(("flaresolverr", _cf_solver_fetch))

    for name, fn in attempts:
        try:
            print(f"[miruro] pipe blocked (HTTP {res.status_code}); trying {name}")
            return await fn(url)
        except Exception as e:
            print(f"[miruro] {name} bypass failed: {e}")

    print(f"[miruro] pipe blocked (HTTP {res.status_code}); set CF_SCRAPER_API_KEY / CF_BRIGHTDATA_API_KEY / CF_TURNSTILE_KEY / CF_SOLVER_URL to enable a bypass")
    return _PipeResponse(res.status_code, res.text)
...
<div class="notice">
  ...
  <span><strong>Hosting Notice:</strong> Miruro now has Cloudflare protection on the pipe endpoint. <strong>Do not deploy on Vercel</strong> — its IPs are datacenter-blocked by CF. Use a <strong>VPS with a residential or non-datacenter IP</strong> instead. <em>Optional bypass:</em> set <code>CF_SOLVER_URL</code> (FlareSolverr-style solver) to fall back to a challenge solver when the pipe is blocked.</span>
</div>
